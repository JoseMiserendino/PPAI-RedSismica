    ...
    namespace PPAI_V2
    {
        internal class GestorRegistrarRM
        {
            ...
            public GestorRegistrarRM(InterfazRegistrarRM interfazRegistrarRM)
            {
                ...
                try
                {
                    ...
                    empleadoLogueado = empleadoDAO.ObtenerPorEmail("juanperez@gmail.com");
                    Console.WriteLine($"Empleado logueado: {empleadoLogueado.nombre} {empleadoLogueado.apellido} (ID: {empleadoLogueado.Id})");
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"Error al cargar empleado: {ex.Message}");
                    // Crear empleado por defecto si no existe
                    empleadoLogueado = new Empleado(1, "Perez", "Juan", "juanperez@gmail.com", "3515555555");
                }

            // Crear sesión
            Sesion.IniciarSesion(empleadoLogueado);
            ...
            // Crear empleado
            Empleado empleado1 = new Empleado(1, "Perez", "Juan", "juanperez@gmail.com", "3515555555");
        ...
        public void TomarEventosAutoDetectadosNoRevisados()
        {
            ...
            eventosAutoDetectadosNoRevisados = eventoDAO.ObtenerPorEstado("AutoDetectado");
            Console.WriteLine("Eventos auto-detectados no revisados obtenidos de la base de datos:" + eventosAutoDetectadosNoRevisados);
            foreach (var evento in eventosAutoDetectadosNoRevisados)
            {
                Console.WriteLine($"Evento auto-detectado no revisado: {evento}");
            }
        ...
        private void CargarSismografosDesdeDB()
        {
            try
            {
                SismografoDAO sismografoDAO = new SismografoDAO();
                sismografos = sismografoDAO.ObtenerTodos();
                Console.WriteLine($"Sismógrafos cargados: {sismografos.Count}");

                foreach (var sismografo in sismografos)
                {
                    Console.WriteLine($"  - {sismografo.IdentificadorSismografo}: {sismografo.TomarNombreEstacion()} ({sismografo.SerieTemporal.Count} series)");
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error al cargar sismógrafos: {ex.Message}");
                sismografos = new List<Sismografo>(); // Lista vacía para evitar NullReferenceException
            }
        }
        ...
        {
            AlcanceSismoDAO alcanceDAO = new AlcanceSismoDAO();
            ClasificacionSismoDAO clasificacionDAO = new ClasificacionSismoDAO();
            OrigenDeGeneracionDAO origenDAO = new OrigenDeGeneracionDAO();
            alcancesSismos = alcanceDAO.ObtenerTodos();
            clasificacionesSismos = clasificacionDAO.ObtenerTodos();
            origenesGeneracionSismos = origenDAO.ObtenerTodos();
        ...
        public void TomarModificacionEvento(string alcance, string clasificacion, string origen)
        {
            ...
            eventoSismicoSeleccionado.AlcanceSismo = alcancesSismos.First(a => a.Nombre.Equals(alcance, StringComparison.OrdinalIgnoreCase));
            eventoSismicoSeleccionado.Clasificacion = clasificacionesSismos.First(c => c.Nombre.Equals(clasificacion, StringComparison.OrdinalIgnoreCase));
            eventoSismicoSeleccionado.OrigenGeneracion = origenesGeneracionSismos.First(o => o.Nombre.Equals(origen, StringComparison.OrdinalIgnoreCase));

            // === GUARDAR EN BASE DE DATOS ===
            EventoSismicoDAO eventoDAO = new EventoSismicoDAO();
            eventoDAO.Actualizar(eventoSismicoSeleccionado);

            Console.WriteLine("Evento actualizado correctamente en la base de datos.");
